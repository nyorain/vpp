...
{
	dlg_check_tagged("~SubmitManager", {
		if(!pending_.empty())
...
{
	dlg_check_tagged("SubmitManager::submit(queue)", {
...
{
	dlg_check_tagged("SubmitManager::add", {
		if(queue.device() != device())
			vpp_error("invalid queue given");
	});
...
void SubmitManager::submit(const CommandExecutionState& state)
{
	...
	dlg_check_tagged(":;SubmitManager::submit(state)", {
		if(!queue)
			vpp_error("SubmitManager::submit(state)", "could not find given state");
	});
	...
	CommandExecutionState& newOne)
{
	...
	dlg_check_tagged("SubmitManager::moveStateObserver", {
		if(it == pending_.end())
...
void SubmitManager::removeStateObserver(const CommandExecutionState& state)
{
	...
	dlg_check_tagged("SubmitManager::removeStateObserver", {
		if(it == pending_.end())
...
void CommandExecutionState::submit()
{
	...
	dlg_check_tagged("CommandExecutionState::submit", {
		if(!submitManager_)
			vpp_error("CommandExecutionState::submit", "invalid object");
	});
