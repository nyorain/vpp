...
public:
	...
	/// Frees the given allocation. No effect if the allocation is invalid,
	/// but will cause a warning in debug mode.
...
public:
	...
	/// The next time a new buffer is allocated, these might be added
	/// without any additional buffer to be created.
	/// Can be used to group alloc groups together in a 
	/// deferred-initialization-like manner for less buffer creations (
	/// and possibly less memory allocations).
...
protected:
	...
	std::deque<SharedBuffer> buffers_;
