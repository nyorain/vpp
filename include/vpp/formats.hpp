...
[[nodiscard]] VPP_API bool supportedUsage(vk::FormatFeatureFlags, vk::ImageUsageFlags);
[[nodiscard]] VPP_API bool supportedUsage(vk::FormatFeatureFlags, vk::BufferUsageFlags);

/// Returns whether the format of the given ImageCreateInfo supports the
/// other parameters.
[[nodiscard]] VPP_API bool supported(const Device&, const vk::ImageCreateInfo&,
	vk::FormatFeatureFlags additional = {});

/// Returns whether the given format is valid for the given use case.
[[nodiscard]] VPP_API bool supported(const Device&, vk::Format, vk::BufferUsageFlags,
	vk::FormatFeatureFlags additional = {});

/// Selects the first format from the list that supports the given
/// use case. Returns vk::Format::undefined if none is supported.
[[nodiscard]] VPP_API vk::Format findSupported(const Device&, nytl::Span<const vk::Format>,
...
[[nodiscard]] VPP_API unsigned mipmapLevels(const vk::Extent2D& extent);
[[nodiscard]] VPP_API unsigned mipmapLevels(const vk::Extent3D& extent);
...
[[nodiscard]] VPP_API vk::Extent3D mipSize(vk::Extent3D size, unsigned l);

/// Combines vk::ImageCreateInfo and vk::ImageViewCreatInfo and
/// offers default initializers.
struct VPP_API ViewableImageCreateInfo {
